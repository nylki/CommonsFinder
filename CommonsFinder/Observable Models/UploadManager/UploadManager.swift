        ...
        tasks[id] = Task<Void, Error> {
            ...
            do {
                ...
                case .twoFactorCodeRequired:
                    ...
                    try setPublishingError(for: uploadable.id, error: .twoFactorCodeRequired)
                    bgTask?.setTaskCompleted(success: false)
                    return
                case .emailCodeRequired:
                    // FIXME: actual trigger a re-login when auth failed (eg. due to 2fa, or password change)
                    // and ability to retry/resume
                    try setPublishingError(for: uploadable.id, error: .emailCodeRequired)
                    bgTask?.setTaskCompleted(success: false)
                    return
                ...
                bgTask?.setTaskCompleted(success: false)
                try setPublishingError(for: uploadable.id, error: .error(errorDescription: error.localizedDescription, recoverySuggestion: "Check if you are logged in to your Wikimedia Account."))
                return
            }
    ...
    private func performMultiUploadImpl(id: DraftIDType, bgTask: BGTask? = nil) {
        ...
        tasks[id] = Task<Void, Error> {
            ...
            for uploadable in uploadables {
                ...
                let request = await Networking.shared.api.publish(file: uploadable, csrfToken: csrfToken, startStep: .uploadData)
