...
enum PublishHelpers {
    ...
    static func parseCategories(from wikitext: String) -> [ParsedCategory] {
        wikitext.matches(of: categoryRegex).compactMap { match in
    ...
    static func updateCategories(in wikitext: String, selectedCategories: [String], knownCategories: Set<String>) -> String {
        ...
        for parsed in parsedCategories {
            ...
            let shouldKeep = selectedNormalized.contains(parsed.normalized)
        ...
        var appendedNormalized = Set<String>()
        for category in selectedCategories {
            let normalized = normalizedCategoryName(category)
            guard appendedNormalized.insert(normalized).inserted else { continue }
            guard !existingNormalized.contains(normalized) else { continue }
            appendedLines.append("[[Category:\(category)]]")
        }
    ...
    static func categoryEditSummary(
        ...
        var parts: [String] = []
        parts.append(contentsOf: added.compactMap { normalized in
            guard let name = selectedByNormalized[normalized] else { return nil }
            return "+[[Category:\(name)]]"
        })
        parts.append(contentsOf: removed.compactMap { normalized in
            guard let name = referenceByNormalized[normalized] else { return nil }
            return "-[[Category:\(name)]]"
        })

        let joined = parts.joined(separator: "; ")
        let summary = joined.isEmpty
            ? "Updated categories using \(toolName)"
            : "\(joined) using \(toolName)"
