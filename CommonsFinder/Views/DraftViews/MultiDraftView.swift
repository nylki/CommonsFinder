...
struct MultiDraftView: View {
    ...
    @State private var isShowingFileListSheet = false
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $isShowingTagsPicker) {
            ...
            TagPicker(
                ...
                onEditedTags: { model.info.multiDraft.tags = $0 },
            )
        ...
        //        }
        .sheet(isPresented: $isShowingFileListSheet) {
            MultiDraftOverviewList(multiDraftModel: model)
        }
        .onAppear {
    ...
    private var toolbarContent: some ToolbarContent {
        ...
        ToolbarItem(placement: .automatic) {
            Button("Show File List", systemImage: "list.clipboard") {
                isShowingFileListSheet = true
            }
        }
        ...
        ToolbarItem(placement: .confirmationAction) {
            if let username = account.activeUser?.username, model.info.multiDraft.uploadPossibleStatus == .uploadPossible {
                Button {
                ...
                .confirmationDialog("Start upload to Wikimedia Commons now?", isPresented: $isShowingUploadDialog, titleVisibility: .visible) {
                    Button("Upload", systemImage: "square.and.arrow.up", role: .fallbackConfirm) {
                        model.saveChanges(appDatabase: appDatabase)
                        uploadManager.upload(model.info, username: username)
                        dismiss()
                    }
